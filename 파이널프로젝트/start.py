...
def GetArr(image):
    ...
    img_resize = img_open.resize((256, 256))
    img_data = Kimage.img_to_array(img_resize)
...
@app.post('/index2_login')
async def index(req:Request, id:str=Form(...), pw:str=Form(...)):
    return templates.TemplateResponse("index2.html", {"request":req})
...
@app.get('/seller')
async def seller(req:Request):
    return templates.TemplateResponse("seller.html", {"request":req})

@app.get('/itemUpload')
async def itemUpload1(req:Request):
    return templates.TemplateResponse("itemUpload.html", {"request":req})

@app.post('/itemUpload2')
async def model(req:Request, width:float=Form(...), height:float=Form(...),image: UploadFile=File(...)):
    ...
    return templates.TemplateResponse("itemUpload2.html",  {"request":req, "fruit":fruit, "name":name, "width":width, "height":height, "weight":weight, "grade":grade ,"image":"static/images/"+image.filename})

@app.get('/chat')
async def chat(req:Request):
    return templates.TemplateResponse("chat.html", {"request":req})

@app.get('/chat2')
async def chat2(req:Request):
    return templates.TemplateResponse("chat2.html", {"request":req})
